@extends('layouts.appAdmin')
@section('contenido')
<div class="content-wrapper">
  <!-- Content Header (Page header) -->
 
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
       <h1>Tienda</h1>
        </div>

        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="{{route('home')}}">Home</a></li>
          <li class="breadcrumb-item"><a href="{{route('tienda.index')}}">Consultar</a></li>
            <li class="breadcrumb-item active">Agregar</li>
          </ol>

      </div>
    </div><!-- /.container-fluid -->
  </section>
    <!-- Main content -->
    <section class="content">
      <div class="container-fluid">
        <div class="row">
          <!-- left column -->
          <div class="col-md-12">
            <!-- jquery validation -->


            <div class="card card-secondary">
              <div class="card-header">
                <span class="fas fa-store float-left"></span>
                <h3 class="card-title ml-1">Informaciòn de la tienda {{$tienda->nombreTienda}}</h3>
              <a href="{{route('tienda.edit',$tienda)}}">
                    <span class="fas fa-edit float-right" aria-hidden ="true" ></span>
                </a>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
            
                <div class="card-body">
                    <dl class="row">
                        <dt class="col-sm-4">Fecha de registro</dt>
                    <dd class="col-sm-8">{{$tienda->created_at}}</dd>
                        <dt class="col-sm-4">Correo electrónico</dt>
                        <dd class="col-sm-8">{{$tienda->correo}}</dd>
                        
                        <dt class="col-sm-4">Plan de afiliación</dt>
                    <dd class="col-sm-8">{{$tienda->planAfiliacion->nombre}}</dd>
                        <dt class="col-sm-4">Estado</dt>
                        <dd class="col-sm-8">@if ($tienda->estatus=='A')
                            <span class="badge badge-success">Activo</span> 
                            @endif
                            @if ($tienda->estatus=='I')
                            <span class="badge badge-danger">Inactivo</span> 
                            @endif
                        </dd>
                      </dl>
                    <div class="col-md-6">
                        <span></span> 
                        <span></span>  
                        <span></span>
                        <span></span>       
                    </div>
                  
                </div>
            </div>



            <div class="card card-secondary">
              <div class="card-header">
                  <span class="fas fa-map-marker-alt float-left"></span>
                
                <h3 class="card-title ml-1">Productos<span class="badge badge-info">{{count($tienda->producto)}}</span></h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
            
              @if(count($tienda->producto)==0)
               <div class="card-body text-center">
                <span >{{$tienda->nombreTienda}} no posee productos disponibles.</span>
                </div>
              @endif
              @if(count($tienda->producto)!=0)
              <div class="card-body table-responsive p-0">
                <table class="table table-hover">
                  <thead>
                    <tr>
                      <th>ID</th>
                      <th>Nombre</th>
                      <th>imagen</th>
                      <th>tipo de producto</th>
                      <th>cantidad</th>
                      <th>precio</th>
                      <th>Acción</th>
                    </tr>
                  </thead>
                  <tbody>
                     @foreach ($tienda->producto as $item)
                         
                     <tr>
                          <td class="mailbox-star">{{$item->id}}</td>
                         <td class="mailbox-star">{{$item->nombre}}</td>
                         <td class="mailbox-star">
                            @if($item->imagen->count()<=0)
                               <img style="height:100px; width:100px; border-radius:10px;" src="/imagenes/avatar.png" >
                             @else
                           <img style="height:100px; width:100px; border-radius:10px;" src="{{$item->imagen->random()->url}}" >
                            @endif    
                         </td>

                         <td class="mailbox-star">{{$item->tipoCliente}}</td>
                         <td class="mailbox-star">{{$item->cantidad}}</td>
                
                         <td class="mailbox-star">{{$item->precioActual}}</td>
                         

                         
                         <td class="mailbox-star">
                             <div class="btn-group">
                             

                             <a href="{{route('producto.edit',$item->slug)}}" class="btn btn-default btn-sm">  <span class="fas fa-edit" aria-hidden ="true" ></span></a>

                                 
                                 
                                 <form action="{{route('producto.destroy',$item)}}" method="POST">
                                     @method('DELETE')
                                     @csrf
                                     <button class="btn btn-default btn-sm d-inline float-left" onclick="return confirm('¿Esta seguro que desea eliminar este producto?')"><span class="fas fa-trash-alt" aria-hidden ="true" ></span></button>
                                   </form>
                                 
                                 
                               
                             </div>
                         </td>
                     </tr>
                     @endforeach
                          
                      
                  
                   
                  </tbody>
                </table>
                </div> 

                @endif
               
               
               

            </div>
            <!-- /.card -->
            </div>
          <!--/.col (left) -->
          <!-- right column -->
          <div class="col-md-6">

          </div>
          <!--/.col (right) -->
        </div>
        <!-- /.row -->
      </div><!-- /.container-fluid -->
    </section>
    <!-- /.content -->
  </div>
  <!-- /.content-wrapper -->
@endsection